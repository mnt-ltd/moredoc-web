    ...
    <el-row :gutter="20">
      <el-col :span="scaleSpan" class="doc-left">
        <el-card ref="docMain" shadow="never" class="doc-main">
          <div slot="header" class="clearfix">
            <h1>
              ...
              <div
                class="actions"
                v-if="user.id>0 && (accessDelete || accessRecommend || accessUpdate || accessForbiden)"
              >
                <el-button
                  type="text"
                  icon="el-icon-edit"
          ...
          <div class="moreinfo">
            <div class="share-info">
              ...
              <nuxt-link
                ...
                class="el-link el-link--primary"
              >{{ document.user.username || '匿名用户' }}</nuxt-link>于
        ...
        <el-card v-if="document.id > 0" ref="commentBox" shadow="never" class="mgt-20px">
          ...
          <FormComment :document-id="document.id" @success="commentSuccess" class="mgt-20px" />
      ...
      <el-card shadow="never">
        <el-row>
          <el-col :span="18">
            <el-button-group class="btn-actions">
              <el-tooltip content="文档点评" v-if="isMobile">
                <el-button icon="el-icon-chat-dot-square" @click="gotoComment"></el-button>
              </el-tooltip>
              <el-tooltip content="全屏阅读" class="hidden-xs-only">
                <el-button icon="el-icon-full-screen" @click="fullscreen"></el-button>
              </el-tooltip>
              <el-tooltip :content="favorite.id > 0 ? '取消收藏' : '收藏文档'">
                <el-button v-if="favorite.id > 0" icon="el-icon-star-on" @click="deleteFavorite"></el-button>
                <el-button v-else icon="el-icon-star-off" @click="createFavorite"></el-button>
              </el-tooltip>
              <el-tooltip content="缩小" class="hidden-xs-only">
                <el-button icon="el-icon-zoom-out" :disabled="scaleSpan === 18" @click="zoomOut"></el-button>
              </el-tooltip>
              <el-tooltip content="放大" class="hidden-xs-only">
                <el-button icon="el-icon-zoom-in" :disabled="scaleSpan === 24" @click="zoomIn"></el-button>
              </el-tooltip>
              <el-tooltip content="上一页" class="hidden-xs-only">
                <el-button icon="el-icon-arrow-up" :disabled="currentPage === 1" @click="prevPage"></el-button>
              </el-tooltip>
          ...
          <el-col :span="6" class="text-right hidden-xs-only">
            <el-button icon="el-icon-top" @click="scrollTop">回到顶部</el-button>
          </el-col>
        </el-row>
      </el-card>
    </div>
    <el-dialog title="举报文档" :visible.sync="reportVisible" :width="isMobile ? '95%' : '640px'">
      <FormReport
        ref="reportForm"
        :init-report="report"
        :is-admin="false"
